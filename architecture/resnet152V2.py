...
train = get_data('../crawler/train')
val = get_data('../crawler/test')
...
pretrained_model= resnet_v2.ResNet152V2(include_top=False,
...
history = resnet_model.fit(x_train,y_train, epochs = 100 , validation_data = (x_val, y_val), callbacks=earlystopping)
# bagging
#vision transformer
#outlier dection
...
plt.savefig("resnet152V2_"+ str(now.strftime("%d-%m-%Y_%H:%M:%S")) + ".jpg")
plt.show()
resnet_model.save('resnet152V2_' + str(now.strftime("%d-%m-%Y_%H:%M:%S")))